...
import { Project, ChatMessage, Character } from '../types';
import { collection, onSnapshot, addDoc, query, orderBy, serverTimestamp, limit, updateDoc, doc } from 'firebase/firestore';
import { db } from '../lib/firebase';
import { Send, Bot, User, Sparkles, Wand2, Lightbulb, ChevronRight, X, MessageSquare, StickyNote, Activity, Target, ShieldCheck, RefreshCw, CheckCircle2 } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-full bg-[#0d0d0d] border-l border-white/5 shadow-2xl">
      ...
      <div className="flex border-b border-white/5 p-2 gap-2">
        ...
        >
          <StickyNote className="w-4 h-4" /> Anotações <span className="bg-editorial-accent/20 text-editorial-accent px-1.5 rounded-md ml-1">12</span>
        </button>
      </div>
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'assistant' ? (
            ...
            >
               ...
               <h3 className="text-[10px] font-black uppercase tracking-widest text-white/40">Área de Anotações Cronológicas</h3>
               <p className="text-[8px] text-white/20 uppercase font-bold leading-relaxed max-w-[200px] mx-auto">Salve ideias rápidas e referências para uso futuro.</p>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
